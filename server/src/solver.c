...
/**
 * Custom SQL function: new_token(user, role, token, max_age). Rebuilds
 ...
 * self-explanatory without relying on a comment that could drift out of
 * sync. Here 'token' is the row's own (pre-update) token column: empty
 * means session_build() generates a fresh random one; non-empty means
 * it's kept as-is, only 'user'/'role', the expiry and the cookie get
 * refreshed.
 */
static void db_new_token(sqlite3_context *context, int argc, sqlite3_value **argv)
{
    ...
    const char *token = session_build(
        ...
        (const char *)sqlite3_value_text(argv[2]),  // token
